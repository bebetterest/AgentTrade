...
describe("API integration", () => {
  ...
  it("rejects dispute creation after the parent task has been terminated", async () => {
    const publisher = addr("dsp-api-term-1");
    const worker = addr("dsp-api-term-2");
    const task = await createSingleSlotTask(publisher);
    await app!.inject({
      method: "POST",
      url: `/v2/tasks/${task.id}/intentions`,
  ...
  it("allows only third-party supervisors to vote on disputes", async () => {
    const publisher = addr("dsp-vote-1");
    const worker = addr("dsp-vote-2");
    const supervisor = addr("dsp-vote-3");
  ...
  it("allows the non-opener party to submit one counterparty dispute reason", async () => {
    const publisher = addr("dsp-resp-1");
    const worker = addr("dsp-resp-2");
    const outsider = addr("dsp-resp-3");
    const task = await createSingleSlotTask(publisher);
    ...
    const workerRespondRes = await app!.inject({
      ...
      headers: { authorization: `Bearer ${bearer(worker)}` },
      payload: { reasonMd: "counterparty reason" }
    });
    ...
    expect(workerRespond.counterpartyReasonMd).toBe("counterparty reason");

    const duplicateRespondRes = await app!.inject({
      method: "POST",
      url: `/v2/disputes/${dispute.id}/counterparty-reason`,
      headers: { authorization: `Bearer ${bearer(worker)}` },
      payload: { reasonMd: "counterparty reason duplicate" }
    });
    ...
    const openerRespondRes = await app!.inject({
      ...
      url: `/v2/disputes/${dispute.id}/counterparty-reason`,
      headers: { authorization: `Bearer ${bearer(publisher)}` },
      payload: { reasonMd: "opener cannot respond" }
    });
    ...
    const outsiderRespondRes = await app!.inject({
      ...
      url: `/v2/disputes/${dispute.id}/counterparty-reason`,
      headers: { authorization: `Bearer ${bearer(outsider)}` },
      payload: { reasonMd: "outsider cannot respond" }
    });
  ...
  it("blocks manual confirm while a submission has an open dispute", async () => {
    const publisher = addr("api-confirm-open-dispute-pub");
    const worker = addr("api-confirm-open-dispute-worker");
  ...
  it("blocks manual termination while a task has an open dispute", async () => {
    const publisher = addr("api-terminate-open-dispute-pub");
    const worker = addr("api-terminate-open-dispute-worker");
  ...
  it("blocks banned publishers from active writes and freezes new intake on their remaining tasks", async () => {
    const publisher = addr("bap");
    const workerA = addr("baa");
    const workerB = addr("bab");
    const workerC = addr("bac");
    const workerD = addr("bad");
    const supervisors = [addr("bas1"), addr("bas2"), addr("bas3"), addr("bas4"), addr("bas5")];

    const slotTask = await createSingleSlotTask(publisher);
    const cleanTask = await createSingleSlotTask(publisher);
    const frozenTask = await createSingleSlotTask(publisher);
    ...
    app!.engine.getLedger(publisher).available = 4;
  ...
  it("shows targeted task mentions in todos and lets targets dismiss them in memory mode", async () => {
    const publisher = addr("targeted-memory-publisher");
    const target = addr("targeted-memory-agent");
    const other = addr("targeted-memory-other");
    const missingTarget = addr("targeted-memory-missing");
    const basePayload = {
      title: "targeted memory task",
      descriptionMd: "desc",
      acceptanceCriteria: "criteria",
      deadlineUtc: futureDeadline(),
      displayTimezone: "UTC",
    ...
    const taskRes = await app!.inject({
      ...
      url: "/v2/tasks",
      headers: { authorization: `Bearer ${bearer(publisher)}` },
      payload: { ...basePayload, title: "targeted memory task valid", targetAgentAddresses: [target] }
    });
    expect(taskRes.statusCode).toBe(200);
  ...
  it("groups account todos across action-required and waiting scopes in memory mode", async () => {
    const target = addr("todo-target");
    const otherPublisher = addr("todo-other-publisher");
    const workerB = addr("todo-worker-b");
    const workerC = addr("todo-worker-c");
    ...
    const rejectedTask = await createTask(otherPublisher, { title: "todo-rejected" });
    ...
    const counterpartyTask = await createTask(target, { title: "todo-counterparty" });
    ...
    const pendingReviewTask = await createTask(target, { title: "todo-pending-review", slotsTotal: 2 });
    ...
    const expiredTask = await createTask(target, { title: "todo-expired" });
    app!.engine.getTask(expiredTask.id).deadlineUtc = new Date(Date.now() - 60_000).toISOString();

    const intendedTask = await createTask(otherPublisher, { title: "todo-intended" });
    await addIntention(intendedTask.id, target);

    const waitingReviewTask = await createTask(otherPublisher, { title: "todo-waiting-review" });
    ...
    const waitingNewSubmissionTask = await createTask(target, { title: "todo-waiting-new" });

    const waitingResolutionTask = await createTask(otherPublisher, { title: "todo-waiting-resolution" });
    ...
    expect(payload.selectedType).toBeNull();
