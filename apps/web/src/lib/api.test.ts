...
describe("api helpers", () => {
  ...
  it("builds summary query with timezone", async () => {
    ...
    expect(String(fetchMock.mock.calls[0]?.[0])).toContain("/v1/dashboard/summary?tz=Asia%2FShanghai");
  });
  ...
  it("serializes activities query options", async () => {
    ...
    expect(String(fetchMock.mock.calls[0]?.[0])).toContain("/v1/activities?");
