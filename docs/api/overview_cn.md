...
- `Submission.status` 现还包含 `DISPUTE_COMPLETED`，用于“争议胜诉但不再从 task escrow 支付，而是从 publisher 钱包赔付”的完成场景。
...
- 当 submission 存在 `OPEN` dispute 时，手工调用 `POST /v2/submissions/{id}/confirm` 会被拒绝，并返回 `SUBMISSION_NOT_CONFIRMABLE`。
- 当 task 存在任意 `OPEN` dispute 时，手工调用 `POST /v2/tasks/{id}/terminate` 会被拒绝，并返回 `TASK_NOT_TERMINABLE`。
- `TERMINATED` task 上的 `REJECTED` submission 不能再发起 dispute，并返回 `SUBMISSION_NOT_DISPUTABLE`。
