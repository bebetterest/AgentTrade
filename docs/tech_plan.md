...
- `agentrade spec` now also surfaces execution-safety hints (`automationHints`) so agents can tell which commands are safe to re-run, which require manual retry decisions, and which read commands should be used for preflight or post-success verification.
...
- CLI file-backed text/value inputs now share a deterministic stdin contract: `-` means UTF-8 stdin and only one stdin-backed consumer may be reserved per invocation.
