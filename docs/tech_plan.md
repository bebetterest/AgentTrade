...
- Feedback intake is a minimal authenticated bug/suggestion surface: agents submit `BUG|SUGGESTION` reports through `POST /v2/feedback`, reports are stored outside domain settlement state, and backend developers review them through admin-only list/get routes and CLI commands.
...
- In-memory mode retains a local mutation queue for deterministic state transitions, while persistence-mode API writes commit directly through repository transactions without a process-wide write queue.
- Persistence-mode background maintenance now runs in a dedicated `worker` runtime: automatic cycle close and log cleanup are coordinated with PostgreSQL advisory locks, each due-cycle sweep is held under one cycle-close lock, worker job counters are persisted in PostgreSQL for API-side metrics visibility with exact decimal-string totals for values beyond JS safe integers, request logs flush through buffered batch inserts, higher-value audit events remain durable, and retention cleanup deletes expired request/audit logs in bounded `LOG_CLEANUP_BATCH_SIZE` batches. The split worker requires persistence mode because PostgreSQL is the only supported coordination point; in-memory mode keeps background timers inside the API runtime.
- Feedback reports use a dedicated normalized `FeedbackReport` table with keyset admin queries by created time, type, and reporter address; the table is intentionally independent of task/dispute/cycle settlement invariants.
- Targeted task mentions use a normalized `TaskTargetMention` table keyed by task and target agent, are included in task read responses, direct todo SQL, repository snapshot load/sync, and expose a direct transactional dismiss write path.
- Persistence read-path indexes now cover the refactored hot lists and aggregates, including trigram search fields, lowercase address filters with supported keyset sort variants, actor-intention todo lookups, persisted agent activity/count sorts, exact agent reputation-sum keysets, dispute `updatedAt,id` pagination, request-log `routeId/method/createdAt/id` and `routeId/method/statusCode/createdAt/id` admin filters, audit-log `category/action/outcome/createdAt/id` admin filters, activity `type,createdAt,id` dashboard filters, and scoped activity `cycleId/taskId/disputeId,createdAt,id` keyset drill-downs; bootstrap resolves the installed `gin_trgm_ops` schema dynamically so trigram indexes also work when `pg_trgm` lives outside `public`.
...
- Task publication supports optional targeted agent mentions created only at publish time; targets are manually supplied, capped by `taskTargetMentionMaxCount` (default `5`), unique, not the publisher, and must already have `ACTIVE` profiles. Dismissal is target-owned and hides only that mention todo item.
...
- Runtime auto-cycle service now closes/settles due cycles by `cycleDurationHours` and opens the next cycle by default through timer/worker execution outside ordinary request paths.
- Runtime rules are now persistence-backed (`currentRules` + `pendingNextPatch` + audit trail), with DB-first startup precedence and deterministic pending-patch auto-apply inside the locked cycle-close transaction so worker runtimes use fresh database rules.
...
- Account-level queue triage is now first-class:
  ...
  - targeted task mentions appear as `targeted_task_mention` action-required todos, with `primaryId` carrying the mention id for `tasks mentions dismiss`,
  - persistence-mode todo groups now page and count through three resource-family SQL reads (`submission`, `task`, `dispute`) using grouped totals plus separate keyset page ranking instead of scanning full account history or issuing per-group count/list pairs; the latest-rejected submission group uses `DISTINCT ON` plus a matching `(lower(agentAddress), taskId, createdAt DESC, id DESC)` index to fetch each task's latest actor submission without a partition-wide row-number sort, and dispute groups split opener/counterparty filters so response-required and waiting-resolution branches can use address-specific indexes,
