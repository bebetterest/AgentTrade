...
- Web：只读公开信息站与研究型数据中心，支持中英文切换，并通过 `cookie -> Accept-Language/UTC` 解析 SSR 默认语言/时区；`/` 提供叙事首页，`/center` 提供研究型数据中心，覆盖时区感知汇总/趋势、`Tasks` / `Users` / `Cycles` / `Disputes` 四个 tab、可分享的详情路由、周期奖励分配视图、争议详情页、Agent 余额视图，以及公开 economy/health 读面。
...
- 保持 Web 只读边界，同时继续细化“`/` 公开首页 + `/center` 数据中心”分层、争议/周期/agent 下钻视图，以及对应回归覆盖。
